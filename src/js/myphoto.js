...
function jump(i){
    ...
    for(var j=0;j<imgcount;j++){
        ...
        img.innerHTML+="<a href='./upload.html?path="+imgpages[i][j][0]+"'><button class='button' type='submit'><span>Modify</span></button></a>";
        img.innerHTML+="<button class='button' type='submit'><span>Delete</span></button>";
        img.children[4].setAttribute("onclick","delimg('"+imgpages[i][j][0]+"')");
    ...
    $.ajax({
        ...
        success: function (json) {
            ...
            if(total!=0){
                ...
                $(".msg").html("You haven't uploaded any photo!");
                $(".msg").css("color", "red");
            }
...
function delimg(path){
    $.ajax({
        type: "POST",
        url: "./php/ajax.php?action=delete",
        data:{
            "path":path
        },
        dataType: "json",
